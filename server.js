...
io.on('connection', (socket) => {
    ...
    /* Output a log message on the server and send it too the clients */
...
serverLog('a page connected to the server: '+socket.id);
   ...
   /* join_room_response:
   {
       ...
       'username': the number of users in the chat room
...
socket.on('game_start', (payload) => {
    ...
    if ((typeof requested_user == 'undefined') || (requested_user === null) || (requested_user === "")) {
        response = {
            result: 'fail',
            message: 'client did not request a valid user to engge in play'
        }
        socket.emit('game_start_response', response);
        serverLog('game_start command failed', JSON.stringify(response));
        return;
...
io.in(room).allSockets().then((sockets) => {
   ...
   {
       'result': 'success',
       'room': the user that send the message,
...
socket.on('send_chat_message', (payload) => {
    ...
    if ((typeof message == 'undefined') || (message === null)){
        ...
        socket.emit('send_chat_message__response',response);
        serverLog('send_chat_message command failed', JSON.stringify(response));
        return;
    }
/* Handle the command */
...
/*This will return true if r + dr supportd playing at r and c + dc supports playing as c*/
